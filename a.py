...
def crack3(pt, et):
    pl = [(__HB(x),__HB(y)) for (x,y) in pt]
    en = [(__HB(x), __HB(y)) for (x,y) in et]

    en = [(x[32:64]+x[0:32],y[32:64]+y[0:32]) for (x,y) in en]
    ts = [__E(x[0:32]) for (x,y) in en]
    si = [__F(__E(x[0:32]),__E(y[0:32])) for (x,y) in en]
    r3 = [__F(x[32:64], y[32:64]) for (x,y) in en]
    l0 = [__F(x[0:32], y[0:32]) for (x,y) in pl]
    so = [__RP(__F(x,y)) for (x,y) in zip(r3, l0)]
    ...
    for i in range(lens):
	    ...
	    x = __HB('%2x' % j)[2:8]
	    for k in range(8):
	        y = __F(x, si[i][k*6:k*6+6])
	        z = __F(__SUB(x,k),__SUB(y,k))
		if z == so[i][k*4:k*4+4] :
			t = int(ts[i][k*6:k*6+6],2)
			jm[k][j^t] += 1
   
    k, s = "" , [26,19,52,57,46,22,45,55]
    for i in range(8):
        for j in range(64):
		if jm[i][j] == lens:
			k += __HB('%2x' % j)[2:8] 
    ...
    for i in range(256):
	    ...
	    y = ''.join(w if w != ' ' else '0' for w in y)
            key = des_key(y)
            code = [(des_en(p, key, 3), des_en(q, key, 3)) for (p,q) in pt]
            if code == et : 
		    return y

def test_gen3(num = 3):
    x = ['%8x' % randint(0, (1<<32)-1) for t in range(num)]
    y = ['%8x' % randint(0, (1<<32)-1) for t in range(num)]
    z = ['%8x' % randint(0, (1<<32)-1) for t in range(num)]
    x = [a+b for (a,b) in zip(x,z)]
    y = [a+b for (a,b) in zip(y,z)]
    return zip(x, y)
